...
4. [문제 1: GPU를 인식하지 못하는 컨테이너](#4-문제-1-gpu를-인식하지-못하는-컨테이너)
5. [문제 2: 모델이 GPU에 올라가지 않음 (0/37 layers)](#5-문제-2-모델이-gpu에-올라가지-않음-037-layers)
6. [문제 3: 외부에서 API 접근 불가](#6-문제-3-외부에서-api-접근-불가)
7. [문제 4: Windows 클라이언트의 한글 인코딩 깨짐](#7-문제-4-windows-클라이언트의-한글-인코딩-깨짐)
8. [성능 벤치마크: GPT-OSS vs Google Gemini](#8-성능-벤치마크-gpt-oss-vs-google-gemini)
9. [컨텍스트 윈도우 확장 (8K → 131K)](#9-컨텍스트-윈도우-확장-8k--131k)
10. [최종 구성 및 교훈](#10-최종-구성-및-교훈)
...
*### 이런 분들에게 추천합니다*
- ***API 비용을 줄이고 싶은 스타트업/중소기업***: 월 수십~수백만원의 API 비용을 전기세 수준으로 절감
- ***민감한 데이터를 다루는 조직***: 의료, 법률, 금융 등 데이터가 외부로 나가면 안 되는 경우
- ***무제한 API 호출이 필요한 경우***: Rate Limit 걱정 없이 사내 서비스에 자유롭게 활용
- ***AI 기술을 내재화하고 싶은 팀***: 모델 커스터마이징, 파인튜닝 등 자체 역량 구축
